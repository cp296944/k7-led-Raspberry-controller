...
| tag | what |
|---|---|
| `pi-v0.1` | Pi networking, daemon skeleton, **OTA self-update + CI release pipeline** |
| `pi-v0.3` | lamp link, raw 8266 proxy, shared UI on the LAN, the 9 "free" endpoints |
| `pi-v0.4`–`0.9` | the always-on engine (port of `arduino/src/Effects.cpp`): smooth ramp, feed/maintenance, tracked lunar, acclimation, seasonal shift |
| `pi-v1.0` | settings page, hardening, 18/18 capabilities |
| `pi-v1.1` | Home Assistant REST integration (`homeassistant/k7_lamp/`) |

## Build

```bash
cd pi-bridge
go build ./cmd/k7-pi-bridge                       # host
GOOS=linux GOARCH=arm64 go build ./cmd/k7-pi-bridge   # Raspberry Pi (arm64)
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/cp296944/k7-led-Raspberry-controller/master/pi-bridge/deploy/install.sh | sudo bash
```

...
2. **Soak monitor** (Phase 4). `cmd/k7-pi-bridge/diag.go` — `diagAPI` writes an
...
3. **`/api/update/apply` was too strict.** 0.9.8 required `tag` to exactly match
   the available release → "available update is pi-v1.0.0, not the requested
   pi-v0.9.10" when the page's cached status was stale. Now: only
...
- Verified vs mock: 3×/api/hand → `writes_today.manual:3`; `/api/diag` snapshot
  + soak.log startup line; `apply {confirm:true, tag:"pi-v0.5.0"}` →
  `{applying:"pi-v1.0.0", requested:"pi-v0.5.0"}` (no more 409).

...
func TestCmpTag(t *testing.T) {
	cases := []struct {
		...
		{"pi-v0.1.0", "pi-v0.1.0", 0},
		{"pi-v0.2.0", "pi-v0.1.0", 1},
		{"pi-v0.1.0", "pi-v0.2.0", -1},
		{"pi-v1.0.0", "pi-v0.9.0", 1},
		{"pi-v0.1.10", "pi-v0.1.2", 1},
		{"pi-v0.4.0-rc.1", "pi-v0.4.0", 0}, // suffix dropped
	}
	for _, c := range cases {
...
func TestShaFor(t *testing.T) {
	sums := []byte("abc123  k7-pi-bridge-linux-arm64\ndeadbeef *SHA256SUMS\n")
	got, err := shaFor(sums, "k7-pi-bridge-linux-arm64")
...
func TestCheckAndApply(t *testing.T) {
	...
	mux.HandleFunc("/repos/acme/k7/releases", func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode([]map[string]any{
			{
				"tag_name": "pi-v0.2.0", "prerelease": false, "body": "notes",
				"assets": []map[string]any{
					{"name": assetName, "browser_download_url": self + "/bin"},
					{"name": sumsName, "browser_download_url": self + "/sums"},
				},
			},
			{"tag_name": "pi-v0.1.0", "prerelease": false, "assets": []map[string]any{
				{"name": assetName, "browser_download_url": self + "/bin"},
				{"name": sumsName, "browser_download_url": self + "/sums"},
			}},
		})
	...
	restarted := false
	u := New(Options{
		Repo: "acme/k7", InstallRoot: root, CurrentTag: "pi-v0.1.0",
	...
	if err != nil || filepath.Base(link) != "pi-v0.2.0" {
		t.Fatalf("current -> %q, %v", link, err)
	}
	if b, _ := os.ReadFile(filepath.Join(root, "releases/pi-v0.2.0/k7-pi-bridge")); string(b) != string(binContent) {
		t.Error("installed binary content wrong")
	}
	if b, _ := os.ReadFile(filepath.Join(root, "state/PREVIOUS")); string(b) != "pi-v0.1.0\n" {
...
func TestConfirmAfterStart(t *testing.T) {
	...
	defer cancel()
	u.ConfirmAfterStart(ctx, "pi-v0.2.0", 10*time.Millisecond, func() bool { return true })
	time.Sleep(80 * time.Millisecond)
	if b, _ := os.ReadFile(filepath.Join(root, "state/CONFIRMED")); string(b) != "pi-v0.2.0\n" {

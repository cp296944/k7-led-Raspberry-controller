...
//
// On-disk layout (InstallRoot, default /opt/k7-pi-bridge):
//
//	releases/pi-v0.1.0/k7-pi-bridge
//	releases/pi-v0.2.0/k7-pi-bridge
//	current            -> releases/pi-v0.2.0        (symlink; systemd ExecStart)
//	data/                                            (state; never touched here)
//	state/PREVIOUS      pi-v0.1.0                     (rollback target)
//	state/CONFIRMED     pi-v0.2.0                     (last version that passed self-check)
//
// Flow: Check() asks the GitHub API for the newest pi-v* release on the
...
const (
	assetName  = "k7-pi-bridge-linux-arm64"
	sumsName   = "SHA256SUMS"
	tagPrefix  = "pi-v"
	binaryName = "k7-pi-bridge"
)
...
type Options struct {
	...
	InstallRoot string // e.g. /opt/k7-pi-bridge
	CurrentTag  string // running version tag, e.g. "pi-v0.1.0" ("dev" disables apply)
	HTTPClient  *http.Client
	// RestartFunc runs the service restart. Default: `systemctl restart k7-pi-bridge`.
...
func systemctlRestart(ctx context.Context) error {
	cmd := exec.CommandContext(ctx, "systemctl", "restart", "k7-pi-bridge")
...
// cmpTag compares "pi-vX.Y.Z" tags. Returns -1, 0, 1. Unparseable sorts low.

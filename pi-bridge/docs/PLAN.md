...
`pi-v0.2`, `pi-v0.3`, … to the Pi automatically; you get `v1.0` when the
...
| step | detail |
|---|---|
| 0.1 | Fork `bitbarista/k7-led-controller` → rename `cp296944/k7-led-Raspberry-controller`; clone to `D:\HomeAssistant\K7\` (full clone, replacing the shallow one); `git remote add upstream` the original |
...
| 1.4 | `.github/workflows/pi-bridge.yml` — on tag `pi-v*`: cross-compile arm64, emit binary + `SHA256SUMS`, publish GitHub Release |
| 1.5 | `deploy/install.sh` + `k7-pi-bridge.service` + update timer; `/opt/k7-pi-bridge/{releases,current,data}` symlink scheme |
| 1.6 | Prove the loop: tag `pi-v0.2` on the fork → Pi self-updates within the hour → `/api/version` reports `0.2` |
...
### Phase 3 — The always-on engine  → `pi-v0.4` … `pi-v0.9`
...
|---|---|---|
| `pi-v0.4` | `persistent_controller_clock` + `/api/time` + `logs` | groundwork: the scheduler tick, ring-buffer log, `/api/output/status`, `/api/wifi/signal` |
| `pi-v0.5` | `smooth_ramp` | interpolate between hourly slots; push `0x1005` only on change, ≥2 min cadence, default **off** (write-wear) |
| `pi-v0.6` | `feed_mode` + `maintenance_mode` | timed overrides (1–60 / 1–180 min) that suspend and restore the scheduler |
| `pi-v0.7` | `tracked_lunar` | real moon phase for 22.63 N / 120.30 E, night window overlay |
| `pi-v0.8` | `acclimation` | multi-day progressive ramp; state survives restart |
| `pi-v0.9` | `seasonal_daylength` | photoperiod drift |

**Exit:** 17/18 capabilities; the UI shows every ESP32 control and they all work.

### Phase 4 — Setup page + hardening  → `pi-v1.0`
...
**Exit: 18/18 capabilities. `pi-v1.0`.** This is your "初版 with everything".

### Phase 5 — Home Assistant (`pi-v1.1`, via OTA)
...
1. Fork + rename on GitHub (`cp296944/k7-led-Raspberry-controller`), tell me when
   it exists.

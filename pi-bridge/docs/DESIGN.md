...
On a version tag (`pi-v*`):

1. `GOOS=linux GOARCH=arm64 go build ./pi-bridge/cmd/k7-pi-bridge`
2. produce `k7-pi-bridge-linux-arm64`, `SHA256SUMS`, `version.json`
...
/opt/k7-pi-bridge/
├── releases/
│   ├── pi-v1.0.0/k7-pi-bridge
│   └── pi-v1.1.0/k7-pi-bridge
├── current  → releases/pi-v1.1.0        (symlink)
...
- if newer than running version: download asset → verify SHA256 against
  ...
  `systemctl restart k7-pi-bridge`
...
systemd runs `/opt/k7-pi-bridge/current/k7-pi-bridge`; the updater only needs
write access to `/opt/k7-pi-bridge` and permission to restart its own unit
...
3. create `/opt/k7-pi-bridge/{releases,data}`, `k7bridge` system user
4. fetch the latest release binary (same path as the updater)
5. install `k7-pi-bridge.service` + `k7-pi-bridge-update.timer`
6. install nftables ruleset
7. `systemctl enable --now k7-pi-bridge`

Config file `/opt/k7-pi-bridge/data/config.toml`:
...
smooth_ramp = false
update_repo = "cp296944/k7-led-Raspberry-controller"
...
|---------|-------------|--------------------|
| **v1.0 (M1 + OTA)** | Pi networking · bridge daemon · shared UI on the LAN · raw `:8266` proxy · **self-update client baked in** · CI release pipeline · `install.sh` | From a LAN PC: `http://k7-pi-wifi-controller.local/` loads the UI; Read / Preview / manual output / push-native-schedule all work; `nc <pi> 8266` reaches the lamp; survives reboot. Tagging `pi-v1.0.1` in the fork → the Pi picks it up within the hour. |
...
| GitHub fork | **`cp296944/k7-led-Raspberry-controller`** (fork renamed on GitHub; `pi-bridge` imports `k7tcp` by local `replace`, so the rename doesn't matter) |

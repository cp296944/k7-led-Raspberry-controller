#!/usr/bin/env bash
# Triggered by systemd `OnFailure=` when k7-pi-bridge crash-loops after an OTA
...
log() { logger -t k7-pi-bridge-rollback "$*"; echo "k7-pi-bridge-rollback: $*" >&2; }
...
if [[ ! -x "$ROOT/releases/$previous/k7-pi-bridge" ]]; then

#!/usr/bin/env bash
# Install / update the K7 Pi Bridge on a Raspberry Pi (Debian, arm64).
#
# Run from a checkout:   sudo pi-bridge/deploy/install.sh [options]
#
#   --binary PATH   install this local binary (bootstrap, before any release exists)
#   --tag TAG       install this release tag from GitHub (default: latest pi-v*)
...
for r in data:
    t=r["tag_name"]
    if not t.startswith("pi-v") or r["draft"]: continue
    if r["prerelease"] and channel!="prerelease": continue
    names={a["name"] for a in r["assets"]}
    if "k7-pi-bridge-linux-arm64" not in names or "SHA256SUMS" not in names: continue
...
install_binary() {  # $1 = tag , $2 = source (local path OR "")
  ...
  if [[ -n "$src" ]]; then
    ...
    curl -fsSL "$base/k7-pi-bridge-linux-arm64" -o "$tmp/bin"
    curl -fsSL "$base/SHA256SUMS" -o "$tmp/sums"
    ( cd "$tmp" && cp bin k7-pi-bridge-linux-arm64 && sha256sum -c --ignore-missing sums )
    install -m 0755 -o "$USER_NAME" -g "$USER_NAME" "$tmp/bin" "$reldir/k7-pi-bridge"
  fi
...
if [[ -n "$BINARY" ]]; then
  ...
  [[ -n "$TAG" ]] || die "no matching pi-v* release found on channel '$CHANNEL' (use --binary to bootstrap)"
...
install -m 0644 "$here/k7-pi-bridge.service"              /etc/systemd/system/k7-pi-bridge.service
install -m 0644 "$here/k7-pi-bridge-rollback.service"     /etc/systemd/system/k7-pi-bridge-rollback.service
install -d /etc/polkit-1/rules.d
install -m 0644 "$here/49-k7-pi-bridge.rules"             /etc/polkit-1/rules.d/49-k7-pi-bridge.rules

systemctl daemon-reload
systemctl enable k7-pi-bridge.service >/dev/null

if [[ $START -eq 1 ]]; then
  systemctl restart k7-pi-bridge.service
  sleep 2
